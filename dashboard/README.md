...
这些能力需要本地服务版，即 `http://127.0.0.1:8765/`。
...
- 明日候选股票池：A池龙头、B池容量趋势、C池20cm弹性，并批量增强技术评分
- 市场情绪指数
- 涨跌停温度
- 涨停票板块/概念归因
- 连板梯队
...
看板里的“个股打分计算器”依赖本地服务 API，不适用于纯静态 GitHub Pages 页面。公网页面会自动禁用实时计算器，并提示使用候选池里的预生成技术评分。
...
盘后候选池会先按涨停、连板、成交额、板块强度、20cm特征生成 A/B/C 三类池子，再对每个池前 5 个候选标的调用短线计算器增强技术评分。
